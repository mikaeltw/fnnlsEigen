...
public:
    ...
    Map(PyArrayObject *object)
        : Base(reinterpret_cast<typename MatrixType::Scalar *>(PyArray_DATA(object)),
        ...
        if (!PyArray_ISONESEGMENT(object)) {
            throw std::invalid_argument(
                "Numpy arrays must be in one contiguous segment to be able to be "
                "transferred to an Eigen Map.");
        }
        if (!PyArray_IS_C_CONTIGUOUS(object) && PyArray_NDIM(object) > 1) {
            throw std::invalid_argument(
                "Only C-contiguous numpy arrays are supported if dimension is > 1");
        }
        Py_INCREF(object_);
    }

    virtual ~Map() { Py_DECREF(object_); }
