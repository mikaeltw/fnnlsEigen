...
template <typename T>
inline PyArrayObject *call_fnnls_solver(PyArrayObject *array_Z, PyArrayObject *array_x,
                                        const int max_iterations, const T tolerance) {
    return eigen_wrapper::onedarray<T>(fnnls::fnnls_solver<T>(
        eigen_wrapper::Map<fnnls::MatrixX_<T>>(array_Z),
        eigen_wrapper::Map<fnnls::VectorX_<T>>(array_x), max_iterations, tolerance));
}
...
public:
    ...
    PyArrayObject *call_fnnls_solver_precompute(PyArrayObject *array_Z, PyArrayObject *array_x,
        ...
        if ((ZT_old.rows() != n_rows) || (ZT_old.cols() != n_cols)
            || !(ZT_old - ZT_new).isMuchSmallerThan(tone)) {
        ...
        return eigen_wrapper::onedarray<T>(
            fnnls::fnnls_solver<T>(ZT_new, eigen_wrapper::Map<fnnls::VectorX_<T>>(array_x),
                                   max_iterations, tolerance, ZTZ.data()));
    }
};
